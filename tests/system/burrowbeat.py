...
class Proc(object):
    ...
    sure to stop the process and close the output file 
    when the object gets collected.
    """
...
class TestCase(unittest.TestCase):
    ...
    def start_burrowbeat(self,
                            cmd="../../burrowbeat.test",
                            config="burrowbeat.yml",
                            output="burrowbeat.log",
                            extra_args=[],
                            debug_selectors=[]):
        """
        Starts burrowbeat and returns the process handle. The 
    ...
    def log_contains(self, msg, logfile="burrowbeat.log"):
        """
        Returns true if the given logfile contains the given message.
    ...
    def output_has(self, lines, output_file="output/burrowbeat"):
        """
    ...
    def all_have_fields(self, objs, fields):
        ...
        for field in fields:
            if not all([field in o for o in objs]):
                raise Exception("Not all objects have a '{}' field".format(field))

    def all_have_only_fields(self, objs, fields):
        """
        Checks if the given list of output objects have
        all and only the given fields.
        Raises Exception if not true.
        """
        self.all_have_fields(objs, fields)
        self.all_fields_are_expected(objs, fields)

    def all_fields_are_expected(self, objs, expected_fields, dict_fields=[]):
        """
        ...
        for o in objs:
            for key in o.keys():
                if key not in dict_fields and key not in expected_fields:
                    raise Exception("Unexpected key '{}' found".format(key))
    ...
    def flatten_object(self, obj, dict_fields, prefix=""):
        ...
        for key, value in obj.items():
            if isinstance(value, dict) and prefix + key not in dict_fields:
                ...
                result.update(self.flatten_object(value, dict_fields, new_prefix))
            else:
